...
Node layout relies on four concepts: `direction`, `lane`, `align_with`,
and the `circular` mode. You provide **no coordinates** — the engine
places each node in relative ratios to the container, which makes the
animation responsive without manual calculations.

## `direction`

Six possible values for `direction` (at the root level of the spec). Five place
free-form nodes; the sixth, [`tree`](#tree-mode), lays out a binary tree:
