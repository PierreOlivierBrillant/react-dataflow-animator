...
La seule exception est le [mode `graph`](#mode-graphe), où vous placez chaque
nœud vous-même via `x` / `y` pour dessiner un graphe quelconque.
...
[`graph`](#mode-graphe) vous laisse les placer vous-même :
...
`graph` est l'échappatoire pour un **graphe quelconque** — un diagramme
nœuds-liens sans ordre naturel de flux, de cercle ou d'arbre (un réseau routier,
ou un graphe pondéré pour une démonstration de Dijkstra / A\* / arbre couvrant
minimal). Ici, vous **fournissez** bien des coordonnées : chaque nœud porte un
`x` et un `y`, chacun une fraction de la scène de `0` (bord gauche / haut) à `1`
(bord droit / bas). `lane`, `align_with` et `main` sont ignorés.
